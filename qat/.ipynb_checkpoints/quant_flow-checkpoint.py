...
sys.path.insert(0,"/root/space/projects/reduced_precision/Sparsity/utils_basic/vision/references/classification/")
...
def get_parser():
    ...
    parser.add_argument('--batch-size-onnx', type=int, default=64, help='batch size for onnx: default 1')

    parser.add_argument('--enable_sparse', default=True, type=bool)
...
def prepare_model(
    ...
    # To solve the error, 'RuntimeError: Expected all tensors to be on the same device, but found at least two devices, cuda:0 and cpu!'
    model.cuda()
    if not pretrained:
        if ckpt_path:
        ...
        if enable_sparse:     # Refer to the checkpoint save mode in sparsity_flow.py
            white_list = [quant_nn.modules.quant_linear.QuantLinear,
                          quant_nn.modules.quant_conv.QuantConv2d]
            ASP.init_model_for_pruning(model, whitelist=white_list, verbosity=3)

        model.load_state_dict(checkpoint)

        if enable_sparse:
            assert(ASP.is_sparsity_enabled())
            print("Is_sparsity_enabled: ", ASP.is_sparsity_enabled())
    model.eval()
    # model.cuda()
...
def main(cmdline_args):
    ...
    ## Prune the model and augment the optimizer for sparse training/inference
    ...
    for epoch in range(args.num_finetune_epochs):
        ...
        # Evaluate during the finetuning
        top1_retrain = evaluate(model, criterion, data_loader_test, device="cuda")
        if args.out_dir:
            checkpoint = {
            ...
            if top1_retrain > top1_best:
                top1_best = top1_retrain
                utils.save_on_master(
                    checkpoint,
